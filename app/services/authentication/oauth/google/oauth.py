...
class GoogleOAuthService(BaseOAuthService):
    ...
    def _build_oauth_connection_instance(self, oauth_user: OAuthUser, internal_user: User) -> GoogleOAuthConnection:
        return GoogleOAuthConnection(user_id=internal_user.id, google_id=oauth_user.id)
